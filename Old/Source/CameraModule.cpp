...
bool CameraModule::Init(Data& settings)
{
	if (App->IsEditor())
	{
		UpdateCameraMatrix(App->windowModule->GetEngineWindowWidth(), App->windowModule->GetEngineWindowHeight());
	}
	else
	{
		UpdateCameraMatrix(App->windowModule->GetGameWindowWidth(), App->windowModule->GetGameWindowHeight());
	}
	return true;
}
