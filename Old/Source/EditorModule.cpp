...
bool EditorModule::Start()
{
	...
	editorWindows.emplace_back(projectSettingsWindow = new ProjectSettingsWindow("Project Settings Window"));

	IMGUI_CHECKVERSION();
	ImGui::CreateContext();
	ImGui_ImplGlfw_InitForVulkan(App->windowModule->window, true);
...
void EditorModule::DrawMainMenu()
{
	...
	ImGui::SameLine(App->windowModule->GetEngineWindowWidth() - 75.f);
...
void EditorModule::DrawFileMenu()
{
	...
	if (ImGui::BeginMenu("Project"))
	{
		...
		{
			projectSettingsWindow->Enable(true);
		}
		ImGui::EndMenu();
	}
}
